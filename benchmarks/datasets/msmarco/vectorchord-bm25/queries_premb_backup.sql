...
CREATE TABLE benchmark_queries (
    query_id INTEGER,
    query_text TEXT,
    query_embedding bm25vector,
    token_bucket INTEGER
);
\copy benchmark_queries (query_id, query_text, token_bucket) FROM 'benchmarks/datasets/msmarco/benchmark_queries.tsv' WITH (FORMAT text, DELIMITER E'\t');
UPDATE benchmark_queries SET query_embedding = tokenize(query_text, 'bert');
...
DECLARE
    q record;
BEGIN
    FOR q IN SELECT query_embedding FROM benchmark_queries ORDER BY random() LIMIT 50 LOOP
        EXECUTE 'SELECT passage_id FROM msmarco_passages
                 ORDER BY embedding <&> to_bm25query(''msmarco_bm25_idx'', $1)
                 LIMIT 10' USING q.query_embedding;
    END LOOP;
END;
$$;
...
BEGIN
    ...
    FOR q IN SELECT query_embedding FROM benchmark_queries WHERE token_bucket = bucket ORDER BY query_id LOOP
        start_ts := clock_timestamp();
        EXECUTE 'SELECT COUNT(*) FROM (SELECT passage_id FROM msmarco_passages
                 ORDER BY embedding <&> to_bm25query(''msmarco_bm25_idx'', $1)
                 LIMIT 10) t' INTO result_count USING q.query_embedding;
...
BEGIN
    ...
    FOR q IN SELECT query_embedding FROM benchmark_queries ORDER BY query_id LOOP
        EXECUTE 'SELECT passage_id FROM msmarco_passages
                 ORDER BY embedding <&> to_bm25query(''msmarco_bm25_idx'', $1)
                 LIMIT 10' USING q.query_embedding;
    END LOOP;
